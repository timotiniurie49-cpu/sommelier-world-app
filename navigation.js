...
window.adminLoadArticles=function(){
  ...
  data.forEach(function(a, idx){
    ...
    btnEdit.title = 'Modifica articolo';
    ...
    (function(id, art, rowEl){
      btnEdit.onclick = function(){
        ...
        window.adminOpenArtEdit(id, art, rowEl);
      };
    })(a.id, a, row);
    ...
    header.appendChild(sp1);
    header.appendChild(sp2);
...
/* Apre form di modifica inline per un articolo */
window.adminOpenArtEdit = function(id, art, rowEl) {
  ...
  var is = 'width:100%;box-sizing:border-box;padding:8px 10px;margin-bottom:8px;' +
    'background:rgba(0,0,0,.3);border:1px solid rgba(212,175,55,.2);' +
    'color:#F5EFE2;border-radius:6px;font-size:13px;font-family:serif;';

  form.innerHTML =
    '<input id="aedit_tit_'+id+'" value="'+(art.titolo_it||'').replace(/"/g,'&quot;')+'" ' +
      'placeholder="Titolo articolo" style="'+is+'">' +
    '<select id="aedit_cat_'+id+'" style="'+is+'">' +
      ['🗞 Attualità del Vino','🌿 Viticoltura Mondiale','🎩 Sommelier del Mondo',
       '🍷 Il Sapere del Vino','🏆 Degustazioni','📚 Didattica per Neofiti'].map(function(c){
        return '<option value="'+c+'"'+(art.categoria_it===c?' selected':'')+'>'+c+'</option>';
      }).join('') +
    '</select>' +
    '<textarea id="aedit_txt_'+id+'" placeholder="Testo articolo..." rows="8" style="'+is+'resize:vertical;">'+(art.testo_it||'')+'</textarea>' +
    '<div style="display:flex;gap:8px;">' +
      '<button onclick="window.adminSaveArtById(this)" data-artid="'+id+'" style="flex:1;padding:9px;' +
        'background:rgba(212,175,55,.15);border:1px solid rgba(212,175,55,.3);color:#D4AF37;' +
        'font-family:Cinzel,serif;font-size:11px;letter-spacing:1px;border-radius:6px;cursor:pointer;">'+
        '💾 SALVA MODIFICHE</button>' +
      '<button onclick="this.parentNode.parentNode.remove()" style="padding:9px 14px;' +
        'background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.1);color:rgba(245,239,226,.4);' +
...
window.adminSaveArt = function(id) {
  /* Find the edit form containing this id */
  var tit = '', cat = '', txt = '';
  document.querySelectorAll('[id^="aedit_tit_"]').forEach(function(el){
    if(el.id === 'aedit_tit_'+id) tit = el.value;
  });
  document.querySelectorAll('[id^="aedit_cat_"]').forEach(function(el){
    if(el.id === 'aedit_cat_'+id) cat = el.value;
  });
  document.querySelectorAll('[id^="aedit_txt_"]').forEach(function(el){
    if(el.id === 'aedit_txt_'+id) txt = el.value;
  });
  if(!tit || !txt) { alert('Titolo e testo sono obbligatori'); return; }

  try {
    var data = JSON.parse(localStorage.getItem('sw_articles')||'[]');
    var idx = data.findIndex(function(a){ return a.id === id; });
    if(idx >= 0) {
      data[idx].titolo_it = tit;
      data[idx].categoria_it = cat;
      data[idx].testo_it = txt;
      data[idx].generato_ai = false; /* ora è manuale */
      localStorage.setItem('sw_articles', JSON.stringify(data));
      if(typeof window.adminLoadArticoli === 'function') window.adminLoadArticoli();
