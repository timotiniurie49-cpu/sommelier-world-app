...
window.i18n = {
  current: (function() {
    try {
      return localStorage.getItem('sw_lang') || 'it';
    } catch(e) {
      return 'it';
    }
  })(),
  dict: {
    it: {
      ...
      homeSelectionTitle:'COLLEZIONE ESCLUSIVA',
      homeSelectionSub:'Private Collection in pre-launch',
      homeSelectionIntro:'Dodici etichette rare, card ampie e immagini ad alta definizione per far respirare la Maison come una vera enoteca di lusso.',
      ...
      somTechnicalDetails:'DETTAGLI TECNICI',
      somTechnicalHint:'Apri il pannello solo se vuoi definire freschezza, carattere e corpo con maggiore precisione.',
      somTechnicalOpen:'Apri i dettagli tecnici',
      somResultIcons:'ICONE DI ABBINAMENTO',
    ...
    en: {
      ...
      homeSelectionTitle:'EXCLUSIVE COLLECTION',
      homeSelectionSub:'Private Collection in pre-launch',
      homeSelectionIntro:'Twelve rare labels, generous cards and high-resolution imagery designed to let the Maison breathe like a luxury wine portal.',
      homeSelectionPrelaunch:'PRE-LAUNCH',
      ...
      somTechnicalDetails:'TECHNICAL DETAILS',
      somTechnicalHint:'Open this panel only if you want to refine freshness, character and body with more precision.',
      somTechnicalOpen:'Open technical details',
      somResultIcons:'PAIRING ICONS',
    ...
    ru: {
      ...
      homeSelectionSub:'Private Collection в pre-launch',
      homeSelectionIntro:'Двенадцать редких бутылок, крупные карточки и high-res still-life, чтобы Maison дышал как винный портал высокого класса.',
      homeSelectionPrelaunch:'PRE-LAUNCH',
      ...
      somTechnicalDetails:'ТЕХНИЧЕСКИЕ ДЕТАЛИ',
      somTechnicalHint:'Откройте панель только если хотите точнее настроить свежесть, характер и тело вина.',
      somTechnicalOpen:'Открыть технические детали',
      somResultIcons:'ИКОНКИ СОЧЕТАНИЯ',
      ...
      prodTitle:'Производители и Maison', prodSub:'Винодельни и виноделы, которым нужен отдельный кураторский зал',
    ...
    fr: {
      ...
      homeSelectionTitle:'COLLECTION EXCLUSIVE',
      homeSelectionSub:'Private Collection en pre-lancement',
      homeSelectionIntro:'Douze etiquettes rares, de grandes cartes et des visuels haute resolution pour laisser respirer la Maison comme un portail d exception.',
      ...
      somTechnicalDetails:'DETAILS TECHNIQUES',
      somTechnicalHint:'Ouvrez ce panneau seulement si vous voulez affiner fraicheur, caractere et corps avec davantage de precision.',
      somTechnicalOpen:'Ouvrir les details techniques',
      somResultIcons:'ICONES D ACCORD',
      manifestoKicker:'THE PHILOSOPHY',
  ...
  try{
    Object.keys(localStorage).forEach(function(k){
      if(k.startsWith('sw_sap_')||k.startsWith('sw_articles')||
         k.startsWith('sw_news')||k.startsWith('sw_trans_')||k.startsWith('sw_build')){
...
function adminWineDBHTML() {
  ...
  html += '<input id="wineAdminSearch" type="text" placeholder="🔍 Cerca per nome, produttore, regione…" '+
    'oninput="adminWineSearch(this.value)" '+
    'style="width:100%;box-sizing:border-box;padding:9px 14px 9px 14px;background:rgba(0,0,0,.3);'+
    'border:1px solid rgba(212,175,55,.25);border-radius:6px;color:#F5EFE2;font-family:Cinzel,serif;'+
    'font-size:.52rem;letter-spacing:.5px;outline:none;">';
...
window.adminWineSearch = function(q) {
  ...
  var filtered = q ? db.filter(function(w){
  ...
  if(countEl) countEl.textContent = (q ? filtered.length+' risultati su '+db.length : db.length)+' vini';

  /* Render lista filtrata */
  var listEl = document.getElementById('wineDbList');
  if(!listEl) return;
  ...
  sortedRegions.forEach(function(regione){
    ...
    wines.forEach(function(w){
      ...
      html += '<div style="padding:7px 10px;background:rgba(255,255,255,.03);border-left:2px solid '+borderColor+';display:flex;align-items:center;gap:6px;">';
      html += '<div style="flex:1;min-width:0;">';
      html += '<div style="font-family:Cinzel,serif;font-size:.52rem;color:rgba(245,239,226,.85);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">'+nome+'</div>';
      html += '<div style="font-size:.7rem;color:rgba(212,175,55,.4);">'+w.produttore+(w.annata&&w.annata!='s.a.'?' — '+w.annata:'')+'</div>';
      html += '</div>';
      html += '<button onclick="adminWineEdit('+rid+')" title="Modifica" style="padding:3px 7px;font-size:.65rem;background:rgba(212,175,55,.08);border:1px solid rgba(212,175,55,.2);color:rgba(212,175,55,.7);border-radius:3px;cursor:pointer;flex-shrink:0;">✏️</button>';
      html += '<button onclick="adminWD('+rid+')" title="Elimina" style="padding:3px 7px;font-size:.65rem;background:rgba(200,50,50,.08);border:1px solid rgba(200,50,50,.2);color:rgba(200,100,100,.7);border-radius:3px;cursor:pointer;flex-shrink:0;">✕</button>';
